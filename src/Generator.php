...
final class Generator
{
    ...
    private function handleEnum(
        ...
        foreach ($definitionNode->values as $value) {
            ...
            $deprecatedDirective = $this->getDirective($value, 'deprecated');
            if ($deprecatedDirective) {
                $deprecated[$value->name->value] = $deprecatedDirective['reason'] ?? '';
            }
        }
        ...
        foreach ($cases as $value) {
            $enumCase = $enum->addCase($value, $value);
            if (isset($deprecated[$value])) {
                $enumCase->addComment(sprintf('@deprecated %s', $deprecated[$value]));
            }
        }
    ...
    private function getDirective(
        FieldDefinitionNode|InputValueDefinitionNode|EnumValueDefinitionNode $field,
        string $name
    ): array|null {
    ...
    private function handleInputObjectType(
        ...
        if (count($definitionNode->fields) > 0) {
            foreach ($definitionNode->fields as $field) {
                ...
                $deprecated = $this->getDirective($field, 'deprecated');
                if ($deprecated) {
                    $method->addComment(sprintf('@deprecated %s', $deprecated['reason'] ?? ''));
                }
    ...
    private function generateFieldArgsInterface(
        ...
        if (count($field->arguments) > 0) {
            foreach ($field->arguments as $argument) {
                ...
                $deprecated = $this->getDirective($argument, 'deprecated');
                if ($deprecated) {
                    $method->addComment(sprintf('@deprecated %s', $deprecated['reason'] ?? ''));
                }
                $method->addComment(sprintf('@return %s', $genericsTypes));
